...
MODULE HYP_2F1_MODULE
  ...
  INTEGER, PARAMETER :: PR=KIND(13),IPR=KIND(1)
...
!----------------------------------------------------------------------
  ...
  COMPLEX(PR)  :: LOG_CONST,I_PI,EPS,LOG_SIN_PI_Z
  COMPLEX(PR),INTENT(OUT) :: RES
  !
...
RECURSIVE FUNCTION GAMMA_INV(Z) RESULT(RES)
  ...
  COMPLEX(PR)  :: GAMMA_SUM,Z_M_0P5,Z_P_G_M0P5,ZM1,EPS
  COMPLEX(PR),INTENT(OUT) :: RES
  !
  X=REAL(Z,PR)
  IF(X.GE.HALF) THEN

...
class Propagator: 
	def __init__(self,a_propagatable):
		...
		self.V0 = self.ShortTimePropagator.V0.clone()
		self.VNow = self.ShortTimePropagator.V0.clone()
		...
		if (Params.DirectionSpecific): 
			...
			#self.Dipoles = numpy.zeros(shape=(self.NSteps,3),dtype = complex)
		else: 
			self.Dipoles = numpy.zeros(shape=(self.NSteps),dtype = float)
			#self.Dipoles = numpy.zeros(shape=(self.NSteps),dtype = complex)
		self.Norms = numpy.zeros(shape=(self.NSteps),dtype = float)
		self.Populations = numpy.zeros(shape=(self.NSteps,Params.dim-1),dtype=float)
		self.Coherences = numpy.zeros(shape=(self.NSteps),dtype = float)
		self.MarkovianRec = numpy.zeros(shape=(self.NSteps,Params.dim-1),dtype=float)
